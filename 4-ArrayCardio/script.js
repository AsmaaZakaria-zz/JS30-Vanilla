...
let f = inventors.filter(inventor => inventor.year < 1600 && inventor.year >= 1500);
console.table(f);

// Array.prototype.map()
// 2. Give us an array of the inventors' first and last names
let m = inventors.map(inventor => inventor.first+" "+inventor.last);
console.table(m);

// Array.prototype.sort()
// 3. Sort the inventors by birthdate, oldest to youngest
let s = inventors.sort((a, b) => a.year - b.year);
console.table(s);

// Array.prototype.reduce()
// 4. How many years did all the inventors live?
let r = inventors.reduce((acc, cur) => acc + (cur.passed - cur.year), 0);
console.log(r);

// 5. Sort the inventors by years lived
let s2 = inventors.sort((a, b) => (b.passed - b.year) - (a.passed - a.year));
console.table(s2);
...
let sum = data.reduce((obj, cur, idx, arr) => {
    if (!obj[cur])
        obj[cur] = 0;
    obj[cur]++;
    return obj;
